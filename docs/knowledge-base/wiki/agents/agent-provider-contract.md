...
|---|---|---|
| Identity | `id`, `name`, `icon`, `logo?` | `id` is a stable key used in settings, dismissals (`"<id>:<sessionId>"`), SSE events, and widget instances. `logo` may be an inline SVG string, a data URL, or a local `/plugin-assets/...` image URL; remote assets are not allowed. Renaming an id orphans user settings — don't. |
