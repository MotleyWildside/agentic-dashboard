...
/**
 * Parsed JSONL log lines are external, loosely-shaped data (Claude transcripts,
 * Codex rollouts) — collectors defensively pick fields off them, so they stay `any`.
 */
export type JsonlLine = any;
 ...
 */
export async function tailJsonl(filePath: string, maxBytes = 256 * 1024): Promise<JsonlLine[]> {
...
export async function headJsonl(filePath: string, maxBytes = 512 * 1024): Promise<JsonlLine | null> {
  const fh = await fs.open(filePath, 'r');
  try {
...
export async function newestFile(
  dirs: string[],
...
export function readJsonSync(filePath: string, maxAgeMs: number | null = null): any {
  try {
    if (maxAgeMs != null) {
...
export function ageSeconds(mtimeMs: number): number {
